...
public class Display extends Application {
    ...
    private SimpleStringProperty receiptButtonText = new SimpleStringProperty("Print Receipts");
    ...
    private void updateReceiptsButton(String text) {
        receiptButtonText.set(text);
    }
    ...
    private void addProcessingFacilities() {
        for (ProcessingFacility processingFacility : Data.getProcessingFacilities()) {
            ...
            for (ProcessingFacility neighbour:processingFacility.getNeighbours()) {
                Line path = new Line(processingFacility.getX() + PATH_OFFSET, processingFacility.getY()
                        + PATH_OFFSET, neighbour.getX() + PATH_OFFSET, neighbour.getY() + PATH_OFFSET);
...
//            System.out.println("Animation completed for the entire path.");
    ...
    /**
     * Triggers reading data for creating processingFacilities, connections, and parcels.
     * Method required for display to run.
     * Complete this in Stage 6: It is
     */
    ...
    /**
     * Adds a button to GUI be able to use it to trigger the printReceipts()
     * Method required for display to run.
     * Complete this in Stage 7.
     * @param vBox to add button to.
     */
    private void addButtonStageSeven(VBox vBox) {
        // create button and assign it to call printReceipts when pressed on and pass down exceptions from printReceipt.
    ...
    /**
     * Outputs the receipt into a txt file.
     * @throws FileNotFoundException File not found.
     */
    private void printReceipts() throws FileNotFoundException {
        String outputFile = "receipts.txt";
        updateReceiptsButton("Print Receipts");
        PrintWriter out = new PrintWriter(outputFile);
        //empty stack in the array list and put it back in the same order.
        ArrayList<Parcel> parcelsArrayList = new ArrayList<>();
        //As long as the processed parcels is not empty pop from stack and print.
        while (!Data.getProcessedParcels().isEmpty()) {
            //popping each parcel out of processed parcel stack and outputting it in the txt file
            Parcel currentParcel = Data.getProcessedParcels().pop();
            out.println(currentParcel);
            parcelsArrayList.add(currentParcel);
        }
        out.close();
        refillStack(parcelsArrayList);
    }

    /**
     * Refills the stack to put it back in the same exact order to be able to print.
     * @param parcels Arraylist of parcels to be put back in place in the same order in the stack.
     */
    private void refillStack(ArrayList<Parcel> parcels) {
        //refill stack in the same order
        for (int i = parcels.size() - 1; i >= 0; i--) {
            Data.getProcessedParcels().push(parcels.get(i));
        }
    }
}
